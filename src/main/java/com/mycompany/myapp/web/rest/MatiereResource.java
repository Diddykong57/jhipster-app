...
public class MatiereResource {
     ...
     */
    @GetMapping("/matieres")
    public List<Matiere> getAllMatieres(@RequestParam(required = false) String filter) {
        if ("controle-is-null".equals(filter)) {
            log.debug("REST request to get all Matieres where controle is null");
            return StreamSupport
                .stream(matiereRepository.findAll().spliterator(), false)
                .filter(matiere -> matiere.getControle() == null)
